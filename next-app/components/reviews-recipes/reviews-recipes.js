...
export default function ReviewSection({ title, buttonName, subtitle, placeholder }) {
  ...
  const loggedInUser = { id: session.user.email, nickname: session.user.name };
    ...
    <>
        ...
        {reviews.map((review) => (
          ...
          >
            <CardBody>
              ...
              {review.description === loggedInUser.id && (
                <div className="flex space-x-2 mt-2">
                  <Button className="w-10 h-5" color="primary" onPress={() => handleOpenModalForEdit(review)}>
                    수정
                  </Button>
                  <Button className="w-10 h-5" color="danger" onPress={() => handleDeleteReview(review.id)}>
                    삭제
                  </Button>
                </div>
              )}
      ...
      <Modal isOpen={isOpen} placement={modalPlacement} onOpenChange={onOpenChange}>
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col">{editingReviewId ? `${subtitle} 수정` : `${subtitle} 작성`}</ModalHeader>
              <ModalBody>
                <div className="flex justify-end mb-4">
                  <StarRating totalStars={5} value={selectedRating} onChange={(value) => setSelectedRating(value)} />
                </div>
                <Textarea
                  isClearable
                  ...
                  onClear={() => setNewReviewText("")}
                />
              </ModalBody>
              <ModalFooter>
                <Button color="danger" variant="light" onPress={onClose}>취소</Button>
                <Button
                  color="bg-primary"
                  onPress={() => {
                    if (editingReviewId) {
                      setReviews((prev) =>
                        prev.map((review) =>
                          review.id === editingReviewId ? { ...review, comment: newReviewText, rating: selectedRating } : review
                        )
                      );
                    } else {
                      const newReview = {
